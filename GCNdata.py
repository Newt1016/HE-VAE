...
class Data(object):
    def __init__(self,args,seed,Markov = False):
        self.name_id = dict() 
        ...
        if self.name in ['Amazon_App','Games']:
            self.epoch = 100
        ...
        if self.name in ['dianping']:
            ...
            self.drop_user,self.drop_item = 25,50
        if self.name in ['Grocery','Kindle','tiktok']:
            self.epoch = 100
            self.entity = ['user','item','G']#'C',
            self.user_side_entity = []
            self.item_side_entity = ['G']#'C'
            self.drop_user,self.drop_item = 50,50
        if self.name in ['tiktok']:
            self.epoch = 50
            self.entity = ['user','item']#'C',
            self.user_side_entity = []
            self.item_side_entity = []#'C'
       ...
       #save 
        ...
        self.train, self.test = self.split_traintest(self.dict_list['user_item'],0.8)
...
#        return train,test,valid
    def split_traintest(self,u_i,ratio):
        ...
        for i,user_item in enumerate(u_i):
            user,item = user_item
            if n <  min(int(user_count[user]*ratio),user_count[user]-2): #
                train.append([user,item])
                n = n + 1
            else:
                test.append([user,item])
    ...
    def find_latest_interaction(self, u_i, keep=last):
        result = dict()
        latest = deque(maxlen=keep)
        for i, user_item in enumerate(u_i):
            user, item = user_item
            result[(user, item)] = list(latest) + [0] * (keep - len(latest))  # 补齐 0
            if i < len(u_i) - 1:
                if u_i[i + 1][0] != user:
                    latest.clear()
                else:
